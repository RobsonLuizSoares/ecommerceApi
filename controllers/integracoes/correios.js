    ...
    try {
        const resultados = await Promise.all(
            config.nCdServico.split(',').map(async (servico) => {
                const resultado = await correios
                    .calcPrecoPrazo({
                        //nCdEmpresa:'código da empresa', para o caso de ter contrato com os Correios
                        // sDsSenha: 'aqui a senha',
                        nCdServico: servico, // no caso do contrato, manter "config.nCdServico"
