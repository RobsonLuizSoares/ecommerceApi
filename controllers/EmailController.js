...
const _send = ({ subject, emails, message }, cb = null) => {
    const mailOptions = {
        from: 'nao-responda@lojanetlink.com.br',
        to: emails,
        subject,
        html: message
    }
...
const enviarNovoPedido = ({ usuario, pedido }) => {
    ...
    <br />
    <p> O pedido realizado hoje, dia ${moment(pedido.createdAt).format('DD/MM/YYYY')}, foi recebido com sucesso! </p>
    <br />
    <a href='${loja}'> Acesse a loja para saber mais. </a>
    <br /><br />
    <p> Atenciosamente,</p>
    <p> Equipe - Loja Net Link </p>
`
    _send({
        subject: 'Pedido Recebido - Loja Net Link',
...
`
    _send({
        subject: 'Pedido Cancelado - Loja Net Link',
        emails: usuario.email,
        message
    })
}
...
`
    _send({
        subject: 'Pedido Atualizado - Loja Net Link',
        emails: usuario.email,
        message
    })
}
